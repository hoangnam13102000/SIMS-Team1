...
-- QUAN TRONG: RoleCode 'CUSTOMER' truoc day CHUA TUNG duoc insert o dau ca,
-- trong khi RegisterFrame/UserDAO.register() luon gan Role.CUSTOMER cho tai
-- khoan tu dang ky. Vi Users.RoleID la NOT NULL, thieu dong nay khien MOI
-- lan dang ky (ke ca lan dau tien) deu insert RoleID = NULL va that bai vi
-- pham NOT NULL constraint -> day la nguyen nhan chinh cua loi
-- "Dang ky that bai, vui long thu lai." sau khi xac nhan OTP.
IF NOT EXISTS (SELECT 1 FROM Roles WHERE RoleCode = 'CUSTOMER')
    INSERT INTO Roles (RoleCode, RoleName, Description)
    VALUES ('CUSTOMER', N'Khách hàng', N'Tự đăng ký qua RegisterFrame, xem sản phẩm/mua hàng ở phía client');
GO

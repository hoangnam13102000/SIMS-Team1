...
public class StockReconciliationDAO extends BaseDAO<StockReconciliation> {
     ...
     */
    /**
     * Lam moi toan bo phien doi chieu cua ngay:
     * - Ton kho cua tung dong = RemainingQty hien tai cua lo trong InventoryBatch.
     * - San pham khong quan ly theo lo = Products.Stock hien tai.
     * - Dat lai toan bo trang thai Da kiem ve chua kiem, xoa nguoi/thoi gian kiem.
     * - Tu dong them cac lo moi phat sinh trong ngay.
     * Khong xoa lich su; chi lam moi cac phieu cua ngay hien tai.
     */
    public int refreshDailySession(LocalDate date, int createdByUserId) {
        if (date == null) return 0;
        try (Connection con = DBConnection.getConnection()) {
            con.setAutoCommit(false);
            try {
                ...
                // Dong gan lo: lay dung ton con lai hien tai cua lo.
                String resetBatchRows =
                        "UPDATE StockReconciliation r " +
                        "LEFT JOIN InventoryBatch b ON r.BatchID = b.BatchID AND b.ProductID = r.ProductID " +
                        "SET r.SystemStock = COALESCE(b.RemainingQty, 0), " +
                        "    r.ActualStock = COALESCE(b.RemainingQty, 0), " +
                        "    r.Checked = 0, r.CheckedBy = NULL, r.CheckedAt = NULL, " +
                        "    r.Note = NULL " +
                        "WHERE r.CreatedAt >= ? AND r.CreatedAt < ? AND r.BatchID IS NOT NULL";

                // Dong khong gan lo: lay ton hien tai cua san pham.
                String resetProductRows =
                        "UPDATE StockReconciliation r " +
                        "JOIN Products p ON p.ProductID = r.ProductID " +
                        "SET r.SystemStock = p.Stock, r.ActualStock = p.Stock, " +
                        "    r.Checked = 0, r.CheckedBy = NULL, r.CheckedAt = NULL, r.Note = NULL " +
                        "WHERE r.CreatedAt >= ? AND r.CreatedAt < ? AND r.BatchID IS NULL";

                try (PreparedStatement ps = con.prepareStatement(resetBatchRows)) {
                    ps.setTimestamp(1, from); ps.setTimestamp(2, to);
                    ps.executeUpdate();
                }
                try (PreparedStatement ps = con.prepareStatement(resetProductRows)) {
                    ps.setTimestamp(1, from); ps.setTimestamp(2, to);
                    ps.executeUpdate();
                }

                // Them cac lo ACTIVE con ton nhung chua co phieu trong phien.
                String insertMissing =
                        "INSERT INTO StockReconciliation " +
                        "(ProductID, BatchID, SystemStock, ActualStock, Note, CreatedBy) " +
                        "SELECT b.ProductID, b.BatchID, p.Stock, p.Stock, NULL, ? " +
                        "FROM InventoryBatch b " +
                        "JOIN Products p ON p.ProductID = b.ProductID " +
                        "JOIN Categories c ON c.CategoryID = p.CategoryID " +
                        "WHERE p.Status = 'ACTIVE' AND c.Status = 'ACTIVE' " +
                        "  AND b.RemainingQty > 0 AND b.Status = 'ACTIVE' " +
                        "  AND NOT EXISTS (SELECT 1 FROM StockReconciliation r " +
                        "      WHERE r.ProductID = b.ProductID AND r.BatchID <=> b.BatchID " +
                        "        AND r.CreatedAt >= ? AND r.CreatedAt < ?)";

                int created = 0;
                try (PreparedStatement ps = con.prepareStatement(insertMissing)) {
                    ps.setInt(1, createdByUserId);
                    ps.setTimestamp(2, from); ps.setTimestamp(3, to);
                    created = ps.executeUpdate();
                }

                // Trigger INSERT cua StockReconciliation co the tinh SystemStock
                // theo Products.Stock. Ghi lai ngay sau INSERT de phieu theo dung
                // RemainingQty cua tung lo, ma khong lam thay doi Products.Stock.
                String syncInsertedBatchRows =
                        "UPDATE StockReconciliation r " +
                        ...
                        "    r.Checked = 0, r.CheckedBy = NULL, r.CheckedAt = NULL " +
                        "WHERE r.CreatedAt >= ? AND r.CreatedAt < ? AND r.BatchID IS NOT NULL";
                try (PreparedStatement ps = con.prepareStatement(syncInsertedBatchRows)) {
                    ps.setTimestamp(1, from); ps.setTimestamp(2, to);
                    ps.executeUpdate();
                }

                // Bao dam san pham khong quan ly theo lo van co dung 1 phieu.
                String insertNonBatch =
                        "INSERT INTO StockReconciliation " +
                        "(ProductID, BatchID, SystemStock, ActualStock, Note, CreatedBy) " +
                        "SELECT p.ProductID, NULL, p.Stock, p.Stock, NULL, ? " +
                        "FROM Products p JOIN Categories c ON c.CategoryID = p.CategoryID " +
                        "WHERE p.Status = 'ACTIVE' AND c.Status = 'ACTIVE' " +
                        "  AND NOT EXISTS (SELECT 1 FROM InventoryBatch b WHERE b.ProductID = p.ProductID) " +
                        "  AND NOT EXISTS (SELECT 1 FROM StockReconciliation r " +
                        "      WHERE r.ProductID = p.ProductID AND r.BatchID IS NULL " +
                        "        AND r.CreatedAt >= ? AND r.CreatedAt < ?)";
                try (PreparedStatement ps = con.prepareStatement(insertNonBatch)) {
                    ps.setInt(1, createdByUserId);
                    ps.setTimestamp(2, from); ps.setTimestamp(3, to);
                    created += ps.executeUpdate();
                }
            ...
            AppLogger.getInstance().error(ErrorCode.DB_UPDATE_FAIL,
                    "StockReconciliationDAO.refreshDailySession date=" + date, e);
            return -1;
        }
    }

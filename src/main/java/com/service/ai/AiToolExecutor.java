...
/**
 * Thực thi tool do Gemini yêu cầu. Luôn kiểm tra quyền trước khi chạm DB. Không
 * bao giờ tin model.
 */
public final class AiToolExecutor {
	...
	public String execute(String toolName, String argsJson, boolean isCustomer) {
		...
		if (!rate.allowed) {
			if (rate.justTriggered) {
				AppLogger.getInstance().error(ErrorCode.AI_CHAT_FAIL,
						"AiTool RATE_LIMIT_EXCEEDED tool=" + toolName + " by=" + identity + " sensitive=" + sensitive,
						null);
			}
			long minutes = Math.max(1, rate.remainingSeconds / 60);
			return "TẠM KHÓA: Bạn đã thao tác quá nhiều lần trong thời gian ngắn. " + "Vui lòng thử lại sau khoảng "
					+ minutes + " phút hoặc liên hệ quản trị viên nếu cần gấp.";
		}
		...
		try {
			...
			AppLogger.getInstance().error(ErrorCode.AI_CHAT_FAIL, "AiToolExecutor.execute - tool=" + toolName, e);
	...
	private String findSimilarProducts(JsonObject args, boolean isCustomer) {
		...
		if (!matched.isEmpty()) {
			return formatProductList("Có sản phẩm tương tự với mô tả \"" + keywords + "\"", matched, isCustomer);
		}
		...
		if (cat != null && cat.isActive()) {
			...
			if (sameCat != null && !sameCat.isEmpty()) {
				...
				sb.append("Gợi ý sản phẩm khác cùng danh mục \"").append(cat.getCategoryName()).append("\":\n");
				sb.append(formatProductListBody(sameCat, isCustomer));
				return sb.toString().trim();
			}
			return "Không tìm thấy sản phẩm khớp với \"" + keywords + "\". " + "Danh mục \"" + cat.getCategoryName()
		...
		return "Không tìm thấy sản phẩm nào khớp với \"" + keywords + "\" và cửa hàng hiện chưa có sản phẩm đang bán.";
	}
	...
	private Category findBestCategoryPartial(String text) {
		...
		int bestScore = 0;
		for (Category c : all) {
			if (c.getCategoryName() == null || !c.isActive())
				continue;
	...
	private String formatProductList(String title, List<Product> list, boolean isCustomer) {
		...
		sb.append(title).append(" (").append(n).append(" kết quả, tối đa ").append(MAX_PRODUCT_RESULTS).append("):\n");
	...
	private String formatProductListBody(List<Product> list, boolean isCustomer) {
		...
		int i = 0;
		for (Product p : list) {
			if (i++ >= MAX_PRODUCT_RESULTS)
				break;
			sb.append("- ").append(p.getProductCode()).append(" | ").append(p.getProductName());
	...
	private String importExcel(JsonObject args, boolean isCustomer) {
		...
		if (!files.isEmpty()) {
			if (files.size() == 1)
				return importer.importFile(files.get(0), type);
	...
	private String getProductDetail(JsonObject args, boolean isCustomer) {
		...
		if (p.getCategoryName() != null)
			sb.append("Danh mục: ").append(p.getCategoryName()).append('\n');
		if (p.getBrand() != null && !p.getBrand().isBlank())
			sb.append("Thương hiệu: ").append(p.getBrand()).append('\n');
		if (p.getUnit() != null)
			sb.append("Đơn vị: ").append(p.getUnit()).append('\n');
		if (p.getWeightVolume() != null)
			sb.append("Quy cách: ").append(p.getWeightVolume()).append('\n');
		...
		if (!isCustomer && canSeeNumericStock()) {
			sb.append("Tồn kho: ").append(p.getStock()).append(" (min: ").append(p.getMinStock()).append(")\n");
		} else {
	...
	private String getStockStatus(JsonObject args, boolean isCustomer) {
		...
		if (!isCustomer && canSeeNumericStock()) {
			return p.getProductCode() + " – " + p.getProductName() + ": tồn " + p.getStock() + " (ngưỡng cảnh báo "
					+ p.getMinStock() + "). " + stockLabel(p);
		}
	...
	private String getEmployeeSalary(JsonObject args) {
		String keyword = text(args, "keyword");
		if (keyword.isBlank())
			return "Vui lòng cung cấp tên hoặc mã nhân viên.";

		PaginationHelper.PaginationResult<Employee> page = employeeDAO.filterByRole(keyword, null, 1, 10);
		...
		for (Employee e : list) {
			if (e.getRole() == Role.CUSTOMER)
				continue;
			sb.append("- ").append(e.getFullName());
			if (e.getEmployeeId() != null)
				sb.append(" (").append(e.getEmployeeId()).append(")");
	...
	private String getRevenueSummary(JsonObject args) {
		...
		try {
			...
			AppLogger.getInstance().error(ErrorCode.AI_CHAT_FAIL, "AiToolExecutor.getRevenueSummary", e);
	...
	private String searchOrders(JsonObject args) {
		...
		PaginationHelper.PaginationResult<Order> page = orderDAO.getPagedFiltered(1, 8,
				keyword.isBlank() ? null : keyword, from, to);
		...
		for (Order o : list) {
			sb.append("- ").append(o.getOrderCode()).append(" | KH: ").append(nullToDash(o.getCustomerName()))
					.append(" | SĐT: ").append(nullToDash(o.getCustomerPhone())).append(" | Tổng: ")
					.append(formatMoney(o.getTotalAmount())).append(" | Đơn: ").append(nullToDash(o.getOrderStatus()))
					.append(" | TT: ").append(nullToDash(o.getPaymentStatus())).append(" | ")
	...
	/**
	 * Tìm 1 đơn hàng theo mã, ưu tiên khớp tuyệt đối trước khi fallback về kết quả
	 * đầu tiên - dùng chung cho get_order_detail và update_order_status.
	 */
	private Order resolveOrderByCode(String code) {
		if (code == null || code.isBlank())
			return null;
		PaginationHelper.PaginationResult<Order> page = orderDAO.getPagedFiltered(1, 5, code, null, null);
		List<Order> list = page != null ? page.getData() : null;
		if (list == null || list.isEmpty())
			return null;
		for (Order o : list) {
			if (o.getOrderCode() != null && o.getOrderCode().equalsIgnoreCase(code.trim())) {
				return o;
			}
		}
		return list.get(0);
	...
	private String getOrderDetail(JsonObject args) {
		...
		sb.append("Thanh toán: ").append(nullToDash(order.getPaymentMethod())).append(" / ")
				.append(nullToDash(order.getPaymentStatus())).append('\n');
		...
		if (details != null && !details.isEmpty()) {
			...
			for (OrderDetail d : details) {
				sb.append("  • ").append(d.getProductName()).append(" x").append(d.getQuantity()).append(" @ ")
						.append(formatMoney(d.getUnitPrice())).append(" = ").append(formatMoney(d.getLineTotal()))
						.append('\n');
			}
		}
		return sb.toString().trim();
	...
	/**
	 * Xác nhận / chuyển trạng thái đơn hàng qua chatbot. Toàn bộ luật nghiệp vụ
	 * (bước chuyển hợp lệ, trừ/hoàn kho FEFO, tự lập hóa đơn khi COMPLETED) nằm ở
	 * {@link OrderDAO#updateOrderStatus(int, String, int, boolean)} - hàm này chỉ
	 * tra đơn theo mã, dịch action sang OrderStatus, và trả lời bằng tiếng Việt dễ
	 * đọc. Quyền ORDER_MANAGE đã được AiTool/AiToolExecutor.execute kiểm tra trước
	 * khi gọi tới đây, nhưng vẫn re-check tại đây theo đúng nguyên tắc "không bao
	 * giờ tin tưởng model" của lớp này.
	 */
	private String updateOrderStatus(JsonObject args) {
		...
		if (actionStr.isBlank())
			return "Thiếu action. Chỉ nhận: CONFIRM, SHIP, COMPLETE hoặc CANCEL.";
		...
		if (newStatus.equals(order.getOrderStatus())) {
			return "Đơn hàng " + order.getOrderCode() + " đã ở trạng thái " + statusLabel(newStatus)
					+ " rồi, không cần cập nhật.";
		}

		var currentUser = AuthService.getInstance().getCurrentUser();
		int actorUserId = currentUser != null ? currentUser.getUserId() : 0;

		OrderDAO.StatusUpdateResult result = orderDAO.updateOrderStatus(order.getOrderId(), newStatus, actorUserId,
				true);
		if (!result.success) {
		...
		return "Đã chuyển đơn hàng " + order.getOrderCode() + " từ " + statusLabel(order.getOrderStatus()) + " sang "
				+ statusLabel(newStatus) + " thành công." + extra
	...
	private String searchInvoices(JsonObject args) {
		...
		PaginationHelper.PaginationResult<Invoice> page = invoiceService.getVisiblePaged(1, 8,
				keyword.isBlank() ? null : keyword, from, to);
		...
		for (Invoice inv : list) {
			sb.append("- ").append(inv.getInvoiceCode()).append(" | KH: ").append(nullToDash(inv.getCustomerName()))
					.append(" | Tổng: ").append(formatMoney(inv.getTotalAmount())).append(" | ")
					.append(nullToDash(inv.getPaymentMethod())).append(" | ").append(nullToDash(inv.getStatus()))
					.append(" | NV: ").append(nullToDash(inv.getCreatedByName())).append(" | ")
					.append(inv.getCreatedAt() != null ? inv.getCreatedAt().toLocalDate() : "—").append('\n');
		}
		return sb.toString().trim();
	}
	...
	private String getInvoiceDetail(JsonObject args) {
		...
		if (code.isBlank())
			return "Thiếu invoice_code.";

		PaginationHelper.PaginationResult<Invoice> page = invoiceService.getVisiblePaged(1, 5, code, null, null);
		List<Invoice> list = page != null ? page.getData() : null;
		Invoice inv = null;
		if (list != null) {
			for (Invoice i : list) {
				if (i.getInvoiceCode() != null && i.getInvoiceCode().equalsIgnoreCase(code.trim())) {
					inv = i;
					break;
				}
			}
			if (inv == null && !list.isEmpty())
				inv = list.get(0);
		}
		if (inv == null) {
		...
		sb.append("VAT: ").append(formatMoney(inv.getVatAmount())).append(" (")
				.append(inv.getVatRate() != null ? inv.getVatRate() + "%" : "—").append(")\n");
		...
		if (details != null && !details.isEmpty()) {
			...
			for (InvoiceDetail d : details) {
				...
				sb.append("  • ").append(name).append(" x").append(d.getQuantity()).append(" @ ")
						.append(formatMoney(d.getUnitPrice())).append(" = ").append(formatMoney(d.getLineTotal()))
						.append('\n');
			}
		}
	...
	private String listCategories(JsonObject args, boolean isCustomer) {
		...
		for (Category c : list) {
			...
			if (!keyword.isBlank()
					&& (c.getCategoryName() == null || !c.getCategoryName().toLowerCase().contains(keyword))) {
				continue;
			}
			n++;
			if (isCustomer) {
				sb.append("- ").append(c.getCategoryName()).append(" | SP đang bán: ").append(c.getActiveProductCount())
						.append('\n');
			} else {
				sb.append("- ID=").append(c.getCategoryId()).append(" | ").append(c.getCategoryName()).append(" | ")
						.append(nullToDash(c.getStatus())).append(" | SP đang bán: ").append(c.getActiveProductCount())
						.append('\n');
			}
		}
		if (n == 0) {
	...
	private String createProduct(JsonObject args) {
		...
		if (name.isBlank())
			return "Thiếu product_name.";
		if (categoryName.isBlank())
			return "Thiếu category_name. Dùng list_categories để xem danh mục.";
		...
		if (minStock < 0)
			return "min_stock không được âm.";
	...
	private Category findCategoryByName(String name) {
		...
		if (all == null)
			return null;
		for (Category c : all) {
			if (c.getCategoryName() != null && c.getCategoryName().trim().equalsIgnoreCase(name.trim())) {
				return c;
			}
		}
	...
	private String createEmployee(JsonObject args) {
		...
		if (fullName.isBlank())
			return "Thiếu full_name.";
		if (email.isBlank())
			return "Thiếu email.";
		if (roleStr.isBlank())
			return "Thiếu role (SALES_STAFF, SALES_MANAGER, INVENTORY_MANAGER, ADMIN).";
		...
		if (result.emailSent) {
			...
			if (result.emailError != null)
				sb.append(" (").append(result.emailError).append(")");
	...
	private String updateCategory(JsonObject args) {
		...
		AppEventBus.getInstance().publish(new DataChangedEvent(DataChangedEvent.CATEGORY));
		return "Đã cập nhật danh mục ID=" + c.getCategoryId() + " | Tên: " + c.getCategoryName() + " | Status: "
				+ c.getStatus();
	}
	...
	private String deleteCategory(JsonObject args) {
		...
		if (count > 0) {
			return "Không thể xóa cứng: danh mục \"" + c.getCategoryName() + "\" còn " + count + " sản phẩm. "
	...
	private String updateProduct(JsonObject args) {
		...
		AppEventBus.getInstance().publish(new DataChangedEvent(DataChangedEvent.PRODUCT));
		return "Đã cập nhật sản phẩm " + p.getProductCode() + " | " + p.getProductName() + " | Giá bán: "
				+ formatMoney(p.getSellPrice()) + " | Tồn: " + p.getStock() + " | Status: " + p.getStatus();
	}
	...
	private String updateEmployee(JsonObject args) {
		String keyword = text(args, "keyword");
		if (keyword.isBlank())
			return "Thiếu keyword (email/tên/mã NV).";
		...
		if (!roleStr.isBlank()) {
			try {
				...
				if (r == Role.CUSTOMER)
					return "Không gán role CUSTOMER cho nhân viên.";
				emp.setRole(r);
			} catch (Exception e) {
		...
		AppEventBus.getInstance().publish(new DataChangedEvent(DataChangedEvent.USER));
		return "Đã cập nhật NV " + emp.getFullName() + " (" + nullToDash(emp.getEmployeeId()) + ")" + " | Role: "
				+ emp.getRole() + " | Status: " + emp.getStatus()
	...
	private String lockEmployee(JsonObject args) {
		...
		AppEventBus.getInstance().publish(new DataChangedEvent(DataChangedEvent.USER));
		return (lock ? "Đã KHÓA" : "Đã MỞ KHÓA") + " tài khoản: " + emp.getFullName() + " ("
				+ nullToDash(emp.getUsername()) + ").";
	}
	...
	private Employee findOneEmployee(String keyword) {
		...
		for (Employee e : staff) {
			if (e.getEmail() != null && e.getEmail().equalsIgnoreCase(keyword.trim()))
				return e;
		}
		for (Employee e : staff) {
			if (e.getEmployeeId() != null && e.getEmployeeId().equalsIgnoreCase(keyword.trim()))
				return e;
		}
	...
	private void logDenied(String toolName, boolean isCustomer) {
		String who = isCustomer ? "CUSTOMER"
				: (AuthService.getInstance().getCurrentUser() != null
						...
						: "UNKNOWN");
		AppLogger.getInstance().error(ErrorCode.AI_CHAT_FAIL, "AiTool DENIED tool=" + toolName + " by=" + who, null);
	}

	/**
	 * Danh tinh dung de tinh rate-limit. Uu tien username that (neu da dang nhap,
	 * ke ca khach da dang ky). Khach chua dang nhap gom chung 1 nhom "GUEST" - han
	 * che vi khong co dinh danh rieng tren desktop app, nhung van chan duoc kieu
	 * spam lien tuc tu cung 1 phien chat.
	 */

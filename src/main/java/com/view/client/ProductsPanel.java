...
public class ProductsPanel extends JPanel {
	...
	private static final int COMPACT_BREAKPOINT = 1100;
	...
	private final JSpinner minPriceSpinner = new JSpinner(new SpinnerNumberModel(0L, 0L, 1_000_000_000L, 10_000L));

	private final JSpinner maxPriceSpinner = new JSpinner(
			new SpinnerNumberModel(1_000_000_000L, 0L, 1_000_000_000L, 10_000L));

	private final JCheckBox inStockOnly = new JCheckBox("Chỉ hiện sản phẩm còn hàng");

	private final JComboBox<String> sortCombo = new JComboBox<>(
			new String[] { "Mặc định", "Tên A - Z", "Giá thấp đến cao", "Giá cao đến thấp" });
	...
	private boolean compactFilterOpen;
	...
	public ProductsPanel() {
		...
		bodyPanel = new JPanel(new BorderLayout(AppSpacing.LG, 0));
		bodyPanel.setOpaque(false);
		bodyPanel.setBorder(new EmptyBorder(0, AppSpacing.XL, AppSpacing.XL, AppSpacing.XL));
		...
		sidebarScrollPane.setPreferredSize(new Dimension(SIDEBAR_WIDTH + 14, 500));

		bodyPanel.add(sidebarScrollPane, BorderLayout.WEST);
		bodyPanel.add(catalogPanel, BorderLayout.CENTER);

		add(LoadingOverlay.attach(bodyPanel, loadingOverlay), BorderLayout.CENTER);
	...
	/**
	 * Goi tu ClientMainFrame khi nguoi dung bam vao 1 the o trang "Danh muc":
	 * chuyen sang trang nay va loc san theo danh muc do.
	 */
			...
			protected void done() {
				if (error != null) {
					...
					try {
						...
						AppLogger.getInstance().error(ErrorCode.DB_QUERY_FAIL, "ProductsPanel.loadCategories - get()",
								e);
						categories = List.of();
					}
				}
	...
	private JComponent buildChip(String label, int categoryId) {
		...
		JLabel chip = new JLabel(label) {
			...
			protected void paintComponent(Graphics g) {
				...
				g2.setColor(active ? SHOP_GREEN : AppColor.BG_LIGHTER);
		...
		chip.setMaximumSize(new Dimension(Integer.MAX_VALUE, 38));
	...
	private void loadProducts() {
		...
		SwingWorker<List<Product>, Void> worker = new SwingWorker<>() {
			...
			protected List<Product> doInBackground() {
				try {
					return categoryIdToLoad == ALL_CATEGORIES ? productDAO.findAllActive()
	...
	/**
	 * Goi tu ClientMainFrame de dieu huong sang trang chi tiet khi bam the san
	 * pham.
	 */
	...
	private void onProductSelected(Product product) {
		if (onProductClickListener != null)
			onProductClickListener.accept(product);
	}
	...
	private void addProductToCart(Product product) {
		...
		if (product.isOutOfStock()) {
			AppAlert.warning(this, "Hết hàng", "Sản phẩm này hiện đã hết hàng.");
			return;
		}

		CartService.getInstance().addToCart(product, 1);

		AppAlert.success(this, "Đã thêm vào giỏ", "\"" + product.getProductName() + "\" đã được thêm vào giỏ hàng.");
	}

	private JComponent buildSidebar() {
		JPanel sidebar = new ResponsiveSidebarPanel();
		sidebar.setLayout(new BoxLayout(sidebar, BoxLayout.Y_AXIS));
		sidebar.setBackground(AppColor.WHITE);

		sidebar.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(AppColor.BORDER),
		...
		applyPriceButton.setMaximumSize(new Dimension(Integer.MAX_VALUE, 38));
	...
	private JComponent buildFilterToolbar() {
		filterToolbar = new JPanel(new BorderLayout(12, 0));
		filterToolbar.setOpaque(false);
		filterToolbar.setBorder(new EmptyBorder(0, 0, 16, 0));

		filterToggleButton = new JButton("☰ Bộ lọc");
		filterToggleButton.setFont(AppFont.SMALL_BOLD);
		filterToggleButton.setForeground(SHOP_GREEN_DARK);
		filterToggleButton.setBackground(SHOP_GREEN_SOFT);
		filterToggleButton.setFocusPainted(false);
		filterToggleButton.setVisible(false);

		filterToggleButton.addActionListener(e -> {
			compactFilterOpen = !compactFilterOpen;

			sidebarScrollPane.setVisible(compactFilterOpen);

			if (compactFilterOpen) {
				sidebarScrollPane.setPreferredSize(new Dimension(10, 300));
			}

			filterToggleButton.setText(compactFilterOpen ? "✕ Đóng bộ lọc" : "☰ Bộ lọc");
		...
		searchField.putClientProperty("JTextField.placeholderText", "Tìm tên, mã hoặc thương hiệu...");
		...
		toolbarRight = new JPanel(new FlowLayout(FlowLayout.RIGHT, 10, 0));

		toolbarRight.setOpaque(false);
		toolbarRight.add(resultLabel);
		toolbarRight.add(new JLabel("Sắp xếp:"));
		toolbarRight.add(sortCombo);

		filterToolbar.add(searchField, BorderLayout.CENTER);
		filterToolbar.add(toolbarRight, BorderLayout.EAST);

		return filterToolbar;
	}

	private void updateResponsiveLayout() {
		if (bodyPanel == null || sidebarPanel == null || sidebarScrollPane == null || filterToolbar == null) {
			return;
		}
		...
		bodyPanel.remove(sidebarScrollPane);
		filterToolbar.removeAll();

		if (compactMode) {
			// ===== CỬA SỔ NHỎ =====
			compactFilterOpen = false;

			sidebarScrollPane.setPreferredSize(new Dimension(10, 300));

			sidebarScrollPane.setVisible(false);

			bodyPanel.setBorder(new EmptyBorder(0, AppSpacing.SM, AppSpacing.LG, AppSpacing.SM));

			bodyPanel.add(sidebarScrollPane, BorderLayout.NORTH);

			filterToggleButton.setVisible(true);
			filterToggleButton.setText("☰ Bộ lọc");

			filterToolbar.setLayout(new BoxLayout(filterToolbar, BoxLayout.Y_AXIS));

			filterToggleButton.setAlignmentX(Component.LEFT_ALIGNMENT);

			searchField.setAlignmentX(Component.LEFT_ALIGNMENT);

			toolbarRight.setAlignmentX(Component.LEFT_ALIGNMENT);

			searchField.setMaximumSize(new Dimension(Integer.MAX_VALUE, 38));

			toolbarRight.setMaximumSize(new Dimension(Integer.MAX_VALUE, 42));

			sortCombo.setPreferredSize(new Dimension(135, 38));

			filterToolbar.add(filterToggleButton);
			filterToolbar.add(Box.createVerticalStrut(8));
			filterToolbar.add(searchField);
			filterToolbar.add(Box.createVerticalStrut(8));
			filterToolbar.add(toolbarRight);

		} else {
			// ===== CỬA SỔ RỘNG =====
			compactFilterOpen = true;
			...
			toolbarRight.setMaximumSize(null);

			sortCombo.setPreferredSize(new Dimension(160, 38));

			filterToolbar.setLayout(new BorderLayout(12, 0));
	...
	private static class ResponsiveSidebarPanel extends JPanel implements Scrollable {

		@Override
		public Dimension getPreferredSize() {
			Dimension size = super.getPreferredSize();

			// Chỉ cố định chiều rộng, chiều cao luôn được tính lại
			// theo số lượng danh mục và các bộ lọc.
			return new Dimension(SIDEBAR_WIDTH, size.height);
		}
	...
	private void applyFilters() {
		...
		long minPrice = ((Number) minPriceSpinner.getValue()).longValue();

		long maxPrice = ((Number) maxPriceSpinner.getValue()).longValue();
		...
		for (Product product : loadedProducts) {
			...
			if (inStockOnly.isSelected() && product.isOutOfStock()) {
				continue;
			}
		...
		case 1 -> filtered
				.sort(Comparator.comparing(product -> safe(product.getProductName()), String.CASE_INSENSITIVE_ORDER));

		case 2 -> filtered.sort(Comparator.comparingLong(this::getProductPrice));

		case 3 -> filtered.sort(Comparator.comparingLong(this::getProductPrice).reversed());
	...
	private boolean matchesKeyword(Product product, String keyword) {
		return safe(product.getProductName()).toLowerCase(Locale.ROOT).contains(keyword)

				|| safe(product.getProductCode()).toLowerCase(Locale.ROOT).contains(keyword)

				|| safe(product.getBrand()).toLowerCase(Locale.ROOT).contains(keyword)

				|| safe(product.getCategoryName()).toLowerCase(Locale.ROOT).contains(keyword);
	}

	private long getProductPrice(Product product) {
		return product.getSellPrice() == null ? 0 : product.getSellPrice().longValue();
	}

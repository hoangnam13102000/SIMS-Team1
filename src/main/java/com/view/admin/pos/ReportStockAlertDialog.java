...
/**
 * Dialog NV/QL bán hàng báo cáo hết hàng / sắp hết hàng thủ công
 * tới Quản lý kho (bảng StockAlerts, ReportedBy = user hiện tại).
 */
public class ReportStockAlertDialog extends JDialog {
    ...
    public ReportStockAlertDialog(Frame owner, Product product, int reportedByUserId) {
    ...
    private JPanel buildBody() {
        ...
        JLabel meta = new JLabel(code + "  ·  Tồn hiện tại: " + product.getStock()
                + "  ·  Tối thiểu: " + product.getMinStock());
        ...
        JLabel hint = new JLabel("<html>Nếu sản phẩm đã có cảnh báo đang xử lý, hệ thống sẽ không tạo thêm bản ghi trùng.</html>");
    ...
    private void submit() {
        ...
        alert.setAlertType(resolveAlertType());
        alert.setStockAtReport(product.getStock());
        String note = noteArea.getText() != null ? noteArea.getText().trim() : "";
        alert.setNote(note.isEmpty() ? null : note);
        alert.setReportedBy(reportedByUserId);

        boolean ok = alertDAO.create(alert);
        if (ok) {
